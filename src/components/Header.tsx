...
export const Header = ({ patientName = "Maria Silva", profilePhoto }: HeaderProps) => {
  ...
  const handleMarkAsRead = async (notification?: Notification) => {
    ...
    try {
      ...
      const response = await fetch("https://appv2-back.samel.com.br/api/notificacao/NotificacaoVisualizada", {
        method: "POST",
        headers: getApiHeaders(),
        body: JSON.stringify({
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      for (const notification of unreadNotifications) {
        await fetch("https://appv2-back.samel.com.br/api/notificacao/NotificacaoVisualizada", {
          method: "POST",
          headers: getApiHeaders(),
          body: JSON.stringify({
            idCliente: idCliente.toString(),
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Avatar className="h-9 w-9 sm:h-11 sm:w-11 border-2 border-primary cursor-pointer hover:opacity-80 transition-opacity order-2 sm:order-1">
                {profilePhoto ? <AvatarImage src={`data:image/jpeg;base64,${profilePhoto}`} alt={patientName} /> : null}
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem
                ...
                onClick={() => window.open("https://cartilha.samel.com.br", "_blank")}
              >
              ...
              <DropdownMenuItem
                className="cursor-pointer"
                onClick={() => window.open("https://www.samel.com.br/politica-de-privacidade/", "_blank")}
              >
              ...
              <DropdownMenuItem
                className="cursor-pointer"
                onClick={() => window.open("https://www.samel.com.br/sobre-nos/faq/", "_blank")}
              >
              ...
              <DropdownMenuItem
                ...
                onClick={() => {
                  ...
                  setTheme(newTheme); // Troca o tema no front-end

                  // NOVO: Notifica o Android da mudança feita pelo usuário
                  if (window.AndroidBridge && typeof window.AndroidBridge.notifyThemeChange === "function") {
                    window.AndroidBridge.notifyThemeChange(newTheme);
                  }
                }}
              >
      ...
      <Dialog open={showNotificationListDialog} onOpenChange={setShowNotificationListDialog}>
        <DialogContent className="max-w-[calc(100vw-1.5rem)] sm:max-w-3xl h-[75vh] sm:h-[90vh] flex flex-col p-0 gap-0">
                  ...
                  <div key={notification.NR_SEQUENCIA}>
                    ...
                    >
                      <div onClick={() => handleNotificationClick(notification)} className="cursor-pointer">
                        ...
                        <p className="text-xs text-muted-foreground">{notification.DATA_FORMATADA}</p>
                      </div>
                      {!notification.DT_VISUALIZADO && (
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2 px-4 sm:px-6 pb-4 sm:pb-6 pt-4 border-t">
            {unreadCount > 0 && (
              <Button onClick={handleMarkAllAsRead} disabled={isMarkingAllAsRead} className="w-full sm:w-auto order-1">
