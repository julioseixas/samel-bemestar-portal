...
export function ExamDetailsDialog({
  ...
  const handleShareWhatsApp = async (exam?: ExamDetail) => {
    ...
    try {
      ...
      const shared = await handlePdfShare(
        pdfBlob, 
        fileName, 
        `Laudo de Exame ${tipoExame}`,
        `Laudo - ${examToShare.procedimentoExame}`
      );
  ...
  return (
    <>
      ...
      >
        <DialogContent className="max-w-[calc(100vw-1.5rem)] max-h-[75vh] sm:max-w-[90vw] sm:h-[90vh] flex flex-col p-0 sm:rounded-lg">
