...
/**
 * AppointmentDetailsV2 - Versão simplificada
 * 
 * Esta versão permite apenas agendamento de UMA especialidade por vez,
 * sem suporte ao fluxo de múltiplas especialidades ou agendamento inteligente.
 */
const AppointmentDetailsV2 = () => {
  ...
  useEffect(() => {
    ...
    fetchEspecialidades();
  }, [selectedConvenio, selectedPatient, useEncaminhamento, navigate, toast]);
  ...
  const handleContinue = async () => {
    ...
    const especialidade = especialidades.find(e => e.id.toString() === selectedEspecialidade);
    if (!especialidade) {
    ...
    try {
      ...
      if (data.sucesso && data.dados) {
        ...
        // Limpar qualquer dado de fluxo convencional que possa existir
